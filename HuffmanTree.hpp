...
private:
    ...
    list<Symbol> lst;
    vector<Data> symbols_in_stock;
    shared_ptr<Node> root;
    weak_ptr<Node> last_leave;
    ...
    inline void insert_new(const unsigned int symbol) {
        shared_ptr<Node> pv = make_shared<Node>(symbol, Side::right);
        shared_ptr<Node> leave = make_shared<Node>(++current_empty_value, Side::left);
...
public:
    ...
    Huffman_Tree() : symbols_in_stock(513) {
        root = make_shared<Node>(++current_empty_value, Side::top);
    ...
    vector<unsigned char> get_symbol_code(const unsigned char symbol) const {
        auto pv = symbols_in_stock[symbol].ptr.lock();

        stack<Side> st;
        vector<unsigned char> symbol_code;
    ...
    vector<unsigned char> get_delim_code() const {
...
private:
    Huffman_Tree &tree;
    weak_ptr<Huffman_Tree::Node> ptr;
public:
