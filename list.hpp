...
private:
    ...
    struct Node {
        ...
        explicit Node(T &&data, Node *prev = nullptr, Node *next = nullptr) noexcept;
...
list<T, Allocator>::Node::Node(const T &data, list::Node *prev, list::Node *next):
        data_(data),
        prev_(prev),
        next_(next) {}

template<typename T, typename Allocator>
list<T, Allocator>::Node::Node(T &&data, list::Node *prev, list::Node *next) noexcept:
        data_(data),
        prev_(prev),
        next_(next) {}
...
template<typename T, typename Allocator>
template<typename... Args>
list<T, Allocator>::Node::Node(list::Node *prev, list::Node *next, Args &&... args):
        prev_(prev),
        next_(next),
...
list<T, Allocator>::list(Allocator alloc):
    ...
    Border_traits::construct(border_alloc, end_);
    Border_traits::construct(border_alloc, rend_);
    rend_->next_ = end_;
    end_->prev_ = rend_;
}
